...
const messages: Record<PennyState, (name?: string, cat?: string, count?: number, names?: string[]) => string> = {
  ...
  newmonth: (name, _cat, count, names) => `Oink! I've copied all your recurring items as individual transactions for ${new Date().toLocaleDateString('en-US', { month: 'long' })}${greet(name)}. No combining allowed! Your ${formatAutoLoggedNames(names)} ${count === 1 ? 'is' : 'are'} already accounted for!`,
};
